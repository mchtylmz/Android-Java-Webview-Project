...
import com.evaavm.R;
import com.evaavm.WebViewAppConfig;
import com.evaavm.ads.AdMobUtility;
import com.evaavm.listener.DrawerStateListener;
import com.evaavm.listener.LoadUrlListener;
import com.evaavm.listener.WebViewOnKeyListener;
import com.evaavm.listener.WebViewOnTouchListener;
import com.evaavm.utility.DownloadFileUtility;
import com.evaavm.utility.IntentUtility;
import com.evaavm.utility.PermissionRationaleHandler;
import com.evaavm.utility.Preferences;
import com.evaavm.view.PullToRefreshMode;
...
public class MainFragment extends TaskFragment implements SwipeRefreshLayout.OnRefreshListener, AdvancedWebView.Listener {
	...
	private String getShareText(String text) {
		if (mWebView != null) {
			...
			Preferences preferences = new Preferences();
			if (mWebView.getUrl().indexOf("sepet") >= 0 && preferences.getOneSignalUserId() != "") {
				text = text.replaceAll("\\{ONE_SIGNAL_USER_ID\\}", preferences.getOneSignalUserId());
			} else  {
				Random rastgele = new Random();
				int rastgeleSayi = rastgele.nextInt(100909)+5;
				text = text.replaceAll("\\{ONE_SIGNAL_USER_ID\\}", Integer.toString(rastgeleSayi));
			}
		}
		return text;
	}
